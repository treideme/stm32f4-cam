...
//  GPIO_Init(GPIOA, &GPIO_InitStructure);
  ...
  //FIXME: Copy image data to circular buffer in RAM
  //SEE: http://www.mind-dump.net/using-the-dma-controller-on-stm32f4
  //FIXME: figure out how to interrupt on DMA buffer overflow
...
void OV7670_config_window(uint16_t startx, uint16_t starty, uint16_t width, uint16_t height)
{
	...
	uint8_t state = 0,temp;

	state = state; // disable compiler warning
